...
/** Editor del contacto visible en la carta pública de una Oferta Académica. */
final class FLACSO_Offer_Carta_Contact_Admin {
    public const META_PERSON_ID = 'asistente_academica_docente_id';
    public const META_TITLE = 'asistente_academica_rol';
    public const META_EMAIL = 'asistente_academica_correo';
    ...
    private static function person_email(int $person_id): string {
        return $person_id > 0
            ? sanitize_email((string) get_post_meta($person_id, 'correo', true))
            : '';
    }

    private static function person_avatar(int $person_id): string {
        return $person_id > 0
            ? (string) (get_the_post_thumbnail_url($person_id, 'thumbnail') ?: '')
            : '';
    }
    ...
    public static function render(WP_Post $post): void {
        ...
        <style>
            ...
            @media(max-width:782px){#flacso_offer_carta_contact .inside{padding:12px}.flacso-carta-contact__fields,.flacso-carta-contact__preview{padding:14px}.flacso-carta-contact__person{align-items:flex-start}}
        </style>

        <p class="flacso-carta-contact__intro">
            <?php esc_html_e('Define quién firma y atiende las consultas en la carta pública de esta Oferta Académica. Esta configuración se mantiene para todas sus cohortes.', 'flacso-uruguay'); ?>
        </p>
        ...
        <div class="flacso-carta-contact">
            <div class="flacso-carta-contact__fields">
                <div class="flacso-carta-contact__field">
                    ...
                    <p class="flacso-carta-contact__help"><?php esc_html_e('Podés elegir cualquier persona del CPT Personas / Equipo, independientemente del rol que tenga en otros grupos.', 'flacso-uruguay'); ?></p>
                </div>
                ...
                <div class="flacso-carta-contact__field">
                    ...
                    <p class="flacso-carta-contact__help"><?php esc_html_e('Es el título visible en la tarjeta; no modifica el rol global de la persona.', 'flacso-uruguay'); ?></p>
                </div>
                ...
                <div class="flacso-carta-contact__field">
                    ...
                    <p class="flacso-carta-contact__help"><?php esc_html_e('Útil cuando una persona atiende distintas ofertas con correos diferentes.', 'flacso-uruguay'); ?></p>
                </div>
            </div>
                ...
                <div class="flacso-carta-contact__preview" aria-live="polite">
                    ...
                    <div class="flacso-carta-contact__person">
                        ...
                        <div>
                            ...
                            <div class="flacso-carta-contact__email" data-contact-email><?php echo esc_html($selected_email !== '' ? $selected_email : __('Se usará el correo configurado actualmente', 'flacso-uruguay')); ?></div>
                        </div>
                    </div>
                </div>
                <div class="flacso-carta-contact__fallback" data-contact-fallback <?php echo $person_id > 0 ? 'hidden' : ''; ?>>
                    <?php esc_html_e('No hay una persona seleccionada. La carta seguirá usando exactamente el fallback que utiliza hoy, por lo que las ofertas existentes no cambian hasta que las configures.', 'flacso-uruguay'); ?>
                </div>
            </div>
        </div>
        ...
        (() => {
            ...
            const update = () => {
                ...
                emailOut.textContent = hasPerson
                    ? (email.value.trim() || personEmail || 'Sin correo cargado en la persona')
                    : 'Se usará el correo configurado actualmente';
