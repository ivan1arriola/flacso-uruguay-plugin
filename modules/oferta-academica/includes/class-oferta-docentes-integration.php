...
/**
 * Gestiona la relacion entre ofertas academicas y docentes.
 */
class Oferta_Docentes_Integration {
    ...
    public static function render_meta_box($post): void {
        ...
        $selected_ids = array_map('intval', self::get_programa_docentes($post->ID));
        ...
        echo '<p><label for="oferta_docentes_search">Selecciona los docentes de la oferta academica:</label></p>';
        echo '<div class="flacso-association-picker" data-picker="docentes">';
        echo '<div class="flacso-association-picker__toolbar">';
        echo '<input type="search" id="oferta_docentes_search" class="flacso-association-picker__search" placeholder="Buscar docente..." autocomplete="off" />';
        ...
        echo '<div class="flacso-association-picker__list" role="group" aria-label="Docentes disponibles">';

        if (!empty($all_docentes)) {
            foreach ($all_docentes as $docente) {
                $docente_id = (int) $docente->ID;
                $title = (string) $docente->post_title;
                $search_value = strtolower(remove_accents($title));
                $checked = isset($selected_map[$docente_id]) ? ' checked' : '';

                echo '<label class="flacso-association-picker__item" data-search="' . esc_attr($search_value) . '">';
                echo '<input type="checkbox" name="oferta_docentes_ids[]" value="' . esc_attr((string) $docente_id) . '"' . $checked . ' />';
                echo '<span>' . esc_html($title) . '</span>';
                echo '</label>';
            }
        } else {
            echo '<p class="description">No hay docentes disponibles.</p>';
        }
    ...
    public static function save_meta($post_id, $post): void {
        ...
        if (isset($_POST['oferta_docentes_ids']) && is_array($_POST['oferta_docentes_ids'])) {
            $docentes_ids = array_values(array_unique(array_map('intval', $_POST['oferta_docentes_ids'])));
    ...
    public static function get_programa_docentes($programa_id): array {
        $value = get_post_meta($programa_id, '_oferta_docentes_ids', true);
        if (!is_array($value)) {
            return [];
        }
        return $value;
