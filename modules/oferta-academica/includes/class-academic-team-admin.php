...
/**
 * Administra la asignación de personas del CPT docente a equipos académicos.
 *
 * Regla de alcance:
 * - Oferta Académica: únicamente el Equipo académico estable.
 * - Cohorte: todos los demás equipos de esa cohorte.
 *
 * Cualquier entrada del CPT `docente` puede pertenecer a cualquier grupo. El rol
 * dentro del grupo se guarda en la relación y no restringe el tipo de persona.
 */
final class FLACSO_Academic_Team_Admin {
    private const NONCE_ACTION = 'flacso_save_academic_teams';
    private const NONCE_NAME = 'flacso_academic_teams_nonce';
        ...
        <div class="flacso-team-editor" data-flacso-team-editor="offer">
            ...
            <?php if (!metadata_exists('post', $post->ID, 'equipo_academico') && metadata_exists('post', $post->ID, 'equipos')) : ?>
                <p class="flacso-team-editor__legacy">
                    <?php esc_html_e('Hay equipos heredados guardados en la Oferta. No se borran automáticamente; esta nueva sección define únicamente el Equipo académico estable.', 'flacso-uruguay'); ?>
                </p>
            <?php endif; ?>
        </div>
        ...
        <div class="flacso-team-editor" data-flacso-team-editor="cohort">
            ...
            <div data-team-list>
                <?php foreach ($teams as $group_index => $team) : ?>
                    <?php self::render_team_group((int) $group_index, $team, $people); ?>
                <?php endforeach; ?>
            </div>
    ...
    public static function render_script(): void {
        ...
        <script>
        (function(){
            function closestEditor(el){ return el.closest('[data-flacso-team-editor]'); }
            function memberTemplate(editor){ return editor.parentElement.querySelector('template[data-member-template]') || document.querySelector('template[data-member-template]'); }
            function nextMemberIndex(container){
                var rows = container.querySelectorAll('[data-member-row]');
                var max = -1;
                rows.forEach(function(row){
                    var select = row.querySelector('select[name]');
                    var match = select && select.name.match(/\[(\d+)\]\[id\]$/);
            ...
            document.addEventListener('click', function(event){
                ...
                if(addMember){
                    ...
                    var editor = closestEditor(addMember);
                    var template = memberTemplate(editor);
                    if(!members || !template){ return; }
                    var index = nextMemberIndex(members);
                    var prefix = members.getAttribute('data-members-prefix') || '';
                    var html = template.innerHTML.replaceAll('__MEMBER_NAME__', prefix + '[' + index + ']');
                ...
                if(addTeam){
                    event.preventDefault();
                    var editor = closestEditor(addTeam);
                    var list = editor.querySelector('[data-team-list]');
                    var template = editor.parentElement.querySelector('template[data-team-template]') || document.querySelector('template[data-team-template]');
                    ...
                    var index = max + 1;
                    var html = template.innerHTML.replaceAll('__GROUP__', String(index));
